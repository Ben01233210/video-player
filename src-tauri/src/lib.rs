...
use std::{
    fs::OpenOptions, io::{BufRead, BufReader, Write}, path::PathBuf, process::Command, time::Duration
};
        ...
        .setup(|app| {
            ...
            match handle_result {
                Ok(handle_wrapper) => {
                    ...
                    let (window_handle, extra_args) = match raw_handle {
                        RawWindowHandle::Win32(handle) => (handle.hwnd.get() as i64, vec![]),
                        RawWindowHandle::Xlib(handle) => (handle.window as i64, vec![]),
                        RawWindowHandle::Xcb(handle) => (handle.window.get() as i64, vec![]),
                        RawWindowHandle::Wayland(handle) => {let i = unsafe {handle.surface.read() };(
                            i as i64,
                            vec![
                        ...
                        RawWindowHandle::AppKit(handle) => (handle.ns_view.as_ptr() as i64, vec![]),
                        _ => {
                    ...
                    std::thread::spawn(move || init(window_handle, extra_args));
...
pub fn init(window_handle: i64, extra_args: Vec<String>) {
    ...
    let mut args: Vec<String> = vec![
        ...
        "--no-osc".to_string(),
        "--hwdec=auto".to_string(), // Hardware decoding for better performance
    ];
...
fn get_videos() -> Vec<String> {
    ...
    let extensions = ["mp4", "mkv", "avi", "mov", "webm", "flv"];

    for entry in WalkDir::new(home_dir).into_iter().filter_map(|e| e.ok()) {
        if entry.file_type().is_file() {
            if let Some(ext) = entry.path().extension() {
                if extensions.contains(&ext.to_str().unwrap_or("")) {
                    videos.push(entry.path().to_str().unwrap().to_string());
                }
            }
        }
    }
